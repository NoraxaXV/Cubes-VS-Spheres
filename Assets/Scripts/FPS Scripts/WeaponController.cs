...
public class WeaponController : MonoBehaviour
{
    ...
    void UpdateAmmo()
    {
        if (m_LastTimeShot + ammoReloadDelay < Time.time && m_CurrentAmmo < maxAmmo && !isCharging && canRecharge)
        {
    ...
    {
        m_CurrentAmmo = Mathf.Clamp(m_CurrentAmmo - Mathf.Round(amount), 0f, maxAmmo);
    ...
    bool TryBeginCharge()
    {
        ...
        {
            UseAmmo(Mathf.Round(ammoUsedOnStartCharge));
            isCharging = true;
